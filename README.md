...
通过此次项目，我对Unity3D游戏引擎的使用有了一个初步的认识，特别感谢复旦大学姜忠鼎教授等老师在Coursera上的游戏开发课程，内容充实，干货挺多，从引擎基础，C#语言，动画物理粒子系统，到AI，光照，性能优化……非常值得一学。
...
尽管如此，在实际开发中也遇到了不少的问题。

比如，课程的Player采用的是胶囊碰撞体+刚体实现，但我在实际测试的时候发现一些bug：

1. 人物撞到墙体后由于是刚体，会受到力的作用弹回来，但在平时的一些游戏中是没有发现过这样的情况的人物撞到墙体后由于是刚体，会受到力的作用弹回来，但在平时的一些游戏中是没有发现过这样的情况的；

2. 某些地方碰撞检测失效

针对这些问题，查了一些资料后改用 CharacterController角色控制器，这样在受制于碰撞的情况下很容易的进行运动，而不用处理刚体。但由此以来，就不能直接用物理API了，所以跳跃或者是对其他物体施力都必须自己写。

总的来说，使用Unity开发游戏还是比较方便了，不过细节较多，所以有时也很繁琐，需要经验积累，如果不常练手的话，很多细节会忘记怎么实现。

相比于写那么多游戏逻辑（游戏逻辑很靠经验！），其实我还是更喜欢研究引擎本身，因为之前学习的计算机图形学的关系，对渲染这一块比较感兴趣，同时，这学期也选修了游戏引擎架构这一门课程，会分析OGRE、Panda3D以及Unreal Engine4，希望学习之后对游戏引擎有个更深的认识！
